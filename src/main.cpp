...
#if defined(NDEBUG)
# error "Ticocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6b0faa6867704663226070de703aec1c07ff38a9feefda8ac8b04dd710628436");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ticocoin: starting difficulty is 1 / 2^12
...
{
    // Ticocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 50000); // Ticocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // Ticocoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Ticocoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ticocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ticocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x24984c5f58c85a87eaa20cd0680d6da8b4029650a2e96ce428c7cb6f15a416f8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "La moneda que un Joven de cucuta colombia quiere impulsar Ing. BRYAN GONZALEZ OROSTEGUI";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 40 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b73f62fb02d038631cc0b500dd90d7dfd8d9426917f032788400cf4e18141b36bb6edfcb7f0dfdbff3784bd5979ffa4aae3b7a5b2c2e2ea98140cef8dfc5c234") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1583876808;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086524757;

        if (fTestNet)
        {
            block.nTime    = 1583876741;
            block.nNonce   = 385844217;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd1bef0a463234b1c15b4c05aad074dffb1569def811978ace13609a81b1ab09e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xca, 0xb3, 0xda }; // Ticocoin: increase each by adding 2 to bitcoin's value.
...
void static TicocoinMiner(CWallet *pwallet)
{
    printf("TicocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ticocoin-miner");
    ...
    {
        printf("TicocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TicocoinMiner, pwallet));
}
